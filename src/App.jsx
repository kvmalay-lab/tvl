...
function App() {
  ...
  const [coachCue, setCoachCue] = useState('Ready');
  ...
  const latestConfRef = useRef(1.0);
  ...
  const ema = (key, value, alpha = 0.35) => {
    if (typeof value !== 'number' || Number.isNaN(value)) return value;
    const s = emaStateRef.current;
    if (!(key in s)) { s[key] = value; return value; }
    s[key] = alpha * value + (1 - alpha) * s[key];
    return s[key];
  };
  // rep segmentation persistence
  const segmenterRef = useRef({ frames: 0, minPersistence: 6 });
  const persist = (cond) => { if (cond) segmenterRef.current.frames++; else segmenterRef.current.frames = 0; return segmenterRef.current.frames >= segmenterRef.current.minPersistence; };
  // keyed persistence counters (multiple independent checks)
  const persistCountersRef = useRef({});
  const persistKey = (key, cond, minFrames = 6) => {
    const pc = persistCountersRef.current;
    if (!pc[key]) pc[key] = 0;
    if (cond) pc[key] += 1; else pc[key] = 0;
    return pc[key] >= minFrames;
  };
  ...
  const detectExerciseReps = (mode, angles) => {
    // angles may include landmarks and optional confidence (angles.conf)
    const confidence = angles.conf || angles.confidence || 1.0;
    const t = thresholdsRef.current || thresholds;
    if (confidence < (t.suggestConf || 0.4) || confidence < (t.minConfidence || 0.2)) {
      setCoachCue('Tracking uncertain — adjust camera or lighting');
      return;
    }
    ...
    const countRep = (note) => {
      repsRef.current += 1;
      setReps(repsRef.current);
      if (note) setCoachCue(note);
      setTimeout(()=> setCoachCue('Keep steady'), 1200);
    };
    ...
    switch (mode) {
      case 'bicep': {
        ...
        if (!angle) return;
        if (persistKey('bicep_down', angle > (t.elbowDown || 160), 6)) {
          if (stageRef.current !== 'down') { stageRef.current = 'down'; setStage('down'); }
        }
        if (persistKey('bicep_up', angle < (t.elbowUp || 35), 6) && stageRef.current === 'down') {
          stageRef.current = 'up'; setStage('up'); countRep('Nice curl — squeeze at the top');
        }
        break;
      }
      case 'squat': {
        ...
        if (!kneeAvg) return;
        if (persistKey('squat_down', kneeAvg < (t.kneeSquat || 100), 6)) {
          if (stageRef.current !== 'down') { stageRef.current = 'down'; setStage('down'); }
        }
        if (persistKey('squat_up', kneeAvg > (t.kneeStand || 160), 6) && stageRef.current === 'down') {
          stageRef.current = 'up'; setStage('up'); countRep('Good depth — control the ascent');
        }
        break;
      }
      case 'pushup': {
        ...
        if (!elAvg) return;
        if (persistKey('pushup_down', elAvg < (t.down || 90), 6)) {
          if (stageRef.current !== 'down') { stageRef.current = 'down'; setStage('down'); }
        }
        if (persistKey('pushup_up', elAvg > (t.up || 160), 6) && stageRef.current === 'down') {
          stageRef.current = 'up'; setStage('up'); countRep('Chest down — drive up with control');
        }
        break;
      }
      default:
    ...
    const initPose = async () => {
      ...
      mediaPipePose.onResults((results) => {
        ...
        if (landmarks && landmarks.length > 0) {
          ...
          // compute simple confidence from visibility where available
          let conf = 0; let confCount = 0;
          [11,12,13,14,15,16,23,24,25,26].forEach(i=>{
            const v = (landmarks[i] && typeof landmarks[i].visibility === 'number') ? landmarks[i].visibility : 1.0;
            conf += v; confCount++;
          });
          conf = confCount ? (conf/confCount) : 1.0;
  ...
  const drawCanvas = (results) => {
    ...
    if (results.poseLandmarks && results.poseLandmarks.length > 0) {
      ...
      // Small detection badge + confidence
      ctx.fillStyle = 'rgba(0,255,0,0.85)';
      ctx.font = '14px sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText('Pose detected', 10, 20);
      const confPct = Math.round((latestConfRef.current || 0) * 100);
      ctx.fillStyle = (latestConfRef.current || 0) < (thresholds.minConfidence || 0.2) ? 'orange' : 'rgba(255,255,255,0.9)';
      ctx.fillText(`Confidence: ${confPct}%`, 10, 40);
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
      ...
      <div className="w-[960px] text-center mb-4">
        <div className="inline-block px-4 py-2 rounded bg-gray-800 text-lg text-yellow-300">{coachCue}</div>
      </div>

      <div className="mb-4">
        <button onClick={() => { setSummary({ reps: reps, avgConf: Math.round((latestConfRef.current||0)*100) }); setShowSummary(true); }} className="px-4 py-2 bg-indigo-600 rounded">End Set / Summary</button>
      </div>

      {showSummary && summary && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50">
          <div className="bg-gray-800 p-6 rounded-lg w-96">
            <h3 className="text-xl font-bold mb-2">Set Summary</h3>
            <p className="mb-2">Reps: <strong>{summary.reps}</strong></p>
            <p className="mb-4">Average tracking confidence: <strong>{summary.avgConf}%</strong></p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowSummary(false)} className="px-3 py-1 bg-gray-700 rounded">Close</button>
            </div>
          </div>
        </div>
      )}
